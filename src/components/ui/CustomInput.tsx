...
const RenderInput = ({
  ...
  field: { onChange: (value: unknown) => void; value: unknown };
  props: InputProps;
}) => {
  switch (props.type) {
    case "input":
    ...
    case "checkbox":
      return (
        <div className="items-top flex space-x-2">
          <Checkbox
            id={props.name}
            onCheckedChange={(e: boolean) => field.onChange(e === true || null)}
          />
...
export const CustomInput = (props: InputProps) => {
  ...
  return (
    <FormField
      control={control}
      name={name}
      render={({ field }: { field: { onChange: (value: unknown) => void; value: unknown } }) => (
...
export const SwitchInput = ({ data, setWorkSchedule }: SwitchProps) => {
  const handleChange = (day: string, field: unknown, value: string) => {
    setWorkSchedule((prevDays) => {
      const dayExist = prevDays.find((d) => d.day === day);

      if (dayExist) {
        return prevDays.map((d) =>
          d.day === day ? { ...d, [field as string]: value } : d
        );
      } else {
        if (field === true) {
          return [
          ...
          return [...prevDays, { day, [field as string]: value }];
        }
      }
    });
  };
